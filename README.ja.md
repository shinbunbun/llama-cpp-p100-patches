...
スケジューラ 2 本、語彙全体のソートを避ける `top_k`、そして参照テーブルの共有メモリ化で
...
**どちらもパッチ 29 より前の測定で、29 を入れて測り直してはいない。** dense のほうは
Q4_1/Q5_1 だけなので影響を受けないが、MoE のほうはバイトの 31.7% が IQ3_XXS なので、
上の値は下限になる。
...
- upstream には未提出。明快な候補が 3 本ある。11 `penalties-direct` /
  ...
  単に手が回っていないからにすぎない
- 全パッチを当てたまま P100 で **`test-backend-ops` が通る**。13,327 件・失敗 0 で、
  同じ機械の無パッチ版と件数も結果も一致する
...
| 29 | `mmvq-iq3xxs-grid-smem` | CUDA | decode +3.1〜7.6%、出力ビット一致 |
