...
**とはいえ、28 本のうち Pascal 世代に限定されているのは 7 本だけである。** もう 1 本は
アーキテクチャで分岐しない定数を変えるので、全 GPU に影響する。残る 20 本はハードウェアに
依存しない。内訳はカーネル融合、添字計算の修正、ホスト側のサンプラー経路 2 本、
スケジューラ 2 本、そして語彙全体のソートを避ける `top_k` である。ただしこのうち 5 本は
gated delta-net を持つモデルでしか発火せず、1 本は特定のモデル機能を必要とする。
...
- upstream には未提出。明快な候補が 3 本ある。11 `penalties-direct` /
  ...
  単に手が回っていないからにすぎない
- 全パッチを当てたまま P100 で **`test-backend-ops` が通る**。13,329 件・失敗 0 を
  3 回連続で確認しており、これは無パッチ版と同じ結果である
...
- Qwen3.5 9B dense と、256 expert の Qwen3.5-MoE (アクティブ 8、41 層)。重みは
  Q4_1 / Q5_1 / IQ 系
