...
**この表は `v0.2.0` へのリベースより前の測定でもあり、`v0.2.0` 上での
無改変 vs パッチ適用は取り直していない。** リベースについて測ったのはアップグレード
そのもの、すなわちパッチ適用 `b10133` とパッチ適用 `v0.2.0` の比較である
(同じカード、`llama-bench`、`-ngl 56 -fa 1`、両腕を交互、初回は捨てて各 3 ラウンド):
9B dense は pp512 +1.15% / tg64 +0.15%、MoE は pp512 −0.03% / tg64 +0.47%。
...
- 全パッチを当てたまま P100 で **`test-backend-ops` が通った**（`v0.2.0` で 13,352 件・
  失敗 0、同じ機械の無パッチ `v0.2.0` 版と件数も結果も一致）。`v0.4.0` では未再測定
- パッチ自身が断っていない限り、出力は無パッチ版と**ビット一致**する。出力が変わるのは
  少数で (09 は K が 4 ワープ中 3 ワープ分を要する場合、12 は通る幅で、15 は設計上)、
  それぞれ根拠を添えて明示してある。22 だけはビット一致の根拠が**論証ではなく実測**である。
  既定の `n_tokens <= 4` は dense と MoE で一致を確認したという事実であって、compute
  バッファのレイアウト差が融合判定を変えないことの証明ではない
...
5. 利用側も同じ変更に含める。API は `lib.llamaCppTag` と `lib.upstreamTag` である。
   ...
   変えた瞬間に黙って壊れる。`v0.2.0` で実際にそうなった。`lib.upstreamTag` は `rev` で
   取得された llama-cpp に対して `null` を返すので、そのまま文字列に埋めないこと
